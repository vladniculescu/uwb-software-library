...
//#define CS_PIN 5
//deckPin_t CS_PIN = {.id=5};
#define CS_PIN DECK_GPIO_IO1
#define LOW 0x0
#define HIGH 0x1

static void deca_sleep(unsigned int delay)
{
  vTaskDelay(M2T(delay));
}

/************ Low level ops for spi **********/
...
static void spi_write(size_t headerLength, const void *header,
...
static void spi_read(size_t headerLength, const void *header,
...
)
{
    ...
    // Write it to the SPI
    spi_write(cnt,header,length,buffer);
...
)
{
    ...
    spi_read(cnt, header, length, buffer);  // result is stored in the buffer
...
float dwt_convertrawvoltage(uint8 raw_voltage)
{
    ...
    realvolt = ((float)(raw_voltage - (float)pdw1000local->vBatP) * (float)SAR_VBAT_TO_VOLT_CONV) + (float)3.3 ;
