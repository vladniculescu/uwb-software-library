...
void uwb_receiver_task(void* parameters) {
    while(1) {
        if(xSemaphoreTake(rxSemaphore, 10 / portTICK_PERIOD_MS)) {
            if(state==RECEIVE) {
                dwt_setrxtimeout(0);
                dwt_rxenable(DWT_START_RX_IMMEDIATE);
            }
        }
        else if((xTaskGetTickCount() - last_time_isr > 10)  && (state==RECEIVE)) {
                // DEBUG_PRINT("X \n");
                dwt_forcetrxoff();
                dwt_setrxtimeout(0);
                dwt_rxenable(DWT_START_RX_IMMEDIATE);
